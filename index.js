...
function areColorsSimilar(rgb1, rgb2, tolerance = 0) {
...
function findClickableNode() {
  return Jimp.read(SCREENSHOT_FILENAME).then(image => {
    ...
    image.scan(area.x, area.y, area.width, area.height, function(x, y, idx) {
      ...
      if (!clickableNode && areColorsSimilar([r, g, b], [156, 148, 116], 5)) {
        clickableNode = [x, y];
      }
    });
...
function isPrestigeReady() {
  return Jimp.read(SCREENSHOT_FILENAME).then(image => {
    const pixels = [[657,563],[677,556],[704,576],[671,612]].map(positions => image.getPixelColor(...positions));

    const allPixelsWhite = pixels.every(pixel => {
      const c = Jimp.intToRGBA(pixel);
      return areColorsSimilar([c.r, c.g, c.b], [255, 255, 255], 5);
    });
